/**
 * Applies the dream-company slug-probe + tier-pack seeding to an existing
 * user's `portals.yml`, merge-not-replace (planning/HUNT2_SOURCES.md §3.2).
 * Core extracted from `scripts/reseedPortals.ts` (ADM-3 Part 0) so it can
 * also run inline, once, right after onboarding completes — the script
 * stays a thin CLI wrapper around this for manual re-seeds.
 *
 ...
 * persists dream_companies) and `profiles.doc`, so it always re-fetches
 * fresh state by `userId` rather than taking it as params — callers must
 * await any prior write to those rows (e.g. `upsertProfileDoc`,
 * `saveSession`) before calling this, or it will act on stale data.
 */
import type { SupabaseClient } from "@supabase/supabase-js";
import yaml from "js-yaml";
import type { Database } from "../supabase/types";
import { seedPortalsCompanies, type TargetingForPortals } from "./portalsSeed";
import { computeTierPack, type CatalogBoardInput } from "../portals/tierPacks";
 ...
 * heuristic remains only for profiles that predate that section. Must
 * stay fallback-only here too — the completion hook is exactly the kind
 * of new call site that hotfix was worried about reintroducing regex as
 * primary.
 */
...
export interface SeedUserPortalsResult {
  dreamCompaniesCount: number;
  tierPackCount: number;
  couldntAutoFind: string[];
  remoteRequired: boolean;
  remoteRequiredSource: "profile.yml" | "regex fallback";
}

export async function seedUserPortals(
  admin: SupabaseClient<Database>,
  userId: string
): Promise<SeedUserPortalsResult> {
  ...
  return {
    dreamCompaniesCount: dreamCompanies.length,
    tierPackCount: tierPackBoards.length,
    couldntAutoFind,
    remoteRequired,
    remoteRequiredSource: locComp ? "profile.yml" : "regex fallback",
  };
}
