...
// record_targeting truncated at the 1536 cap, indistinguishable downstream
// from an empty turn). Additive: exposes the same max_tokens value already
// passed to messages.create(), nothing else changes.
describe("runInterviewTurn — exposes maxTokens on the result", () => {
  it("returns maxTokens matching the cap passed to messages.create", async () => {
    ...
    expect(createMock).toHaveBeenCalledWith(expect.objectContaining({ max_tokens: 1536 }));
  });
});
