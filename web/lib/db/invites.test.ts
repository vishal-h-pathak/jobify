...
describe("claimInvite", () => {
  it("returns true when the update affects a row (code was unclaimed)", async () => {
    const supabase = fakeSupabase({ data: [{ code: "ABC123" }], error: null });
    const claimed = await claimInvite(supabase, "ABC123", "user-1");
    expect(claimed).toBe(true);
  });

  it("returns false when zero rows are updated (invalid or already-claimed code) — this is the invite gate", async () => {
    const supabase = fakeSupabase({ data: [], error: null });
    const claimed = await claimInvite(supabase, "ALREADY-USED", "user-1");
  ...
  it("throws on a database error rather than silently reporting success", async () => {
    ...
    await expect(claimInvite(supabase, "ABC123", "user-1")).rejects.toThrow("boom");
  });
});
