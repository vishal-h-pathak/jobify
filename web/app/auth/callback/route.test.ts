...
describe("GET /auth/callback", () => {
  ...
  it("rejects a protocol-relative next (//evil.com) and falls back to the access-based default", async () => {
  ...
  it("rejects an absolute next (https://evil.com) and falls back to the access-based default", async () => {
  ...
  it("defaults to /invite when there is no next and hasAccess is false (no claim, no allowlist hit, not admin)", async () => {
    hasAccessMock.mockResolvedValue(false);
    const res = await GET(req("/auth/callback?code=abc"));
    expect(res.headers.get("location")).toBe("https://jobify.example/invite");
  });
  ...
  it("2026-07-21 fix: also defaults to /feed for a freshly allowlist-auto-claimed email (hasAccess covers both paths) — the (app) layout's own intake check sends it on to /onboarding from there", async () => {
    hasAccessMock.mockResolvedValue(true);
    const res = await GET(req("/auth/callback?code=abc"));
    expect(res.headers.get("location")).toBe("https://jobify.example/feed");
    expect(hasAccessMock).toHaveBeenCalledWith(expect.anything(), { id: "user-1" });
  });

  it("defaults admins to /admin when there is no next, without checking hasAccess", async () => {
