#!/usr/bin/env bash
# setup-hosted-int1.sh — stage Hosted LIV-1 (living profile) -
...
BRANCH="feat/liv1-learning"
PROMPT="$JOBIFY/planning/session-prompts/35_liv1_living_profile.md"
WT="$WT_ROOT/liv1-learning"
...
tell application "Terminal"
  ...
  delay 0.5
  do script "cd '$WT' && echo '──────── LIV-1 · living profile · SONNET ────────' && claude --permission-mode bypassPermissions --model sonnet" in front window
end tell
OSA
sleep 6
osascript <<OSA
...
OSA
echo "Staged LIV-1 (Sonnet). Review directive, press Return."
