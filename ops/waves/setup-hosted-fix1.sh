#!/usr/bin/env bash
# setup-hosted-fix1.sh — stage Hosted FIX-1 (interview turn bug) -
# parallel-safe with ADM-2 and SGN-1.
# PREREQ: wave 8 (ADM-2) merged to main — this script refuses to run before.
# Run from anywhere:  bash setup-hosted-fix1.sh
set -euo pipefail
JOBIFY="$HOME/dev/jarvis/jobify"; WT_ROOT="$HOME/dev/jarvis/jobify-wt"
BRANCH="feat/hosted-fix1-interview"
PROMPT="$JOBIFY/planning/session-prompts/25_fix_interview_turns.md"
WT="$WT_ROOT/hosted-fix1-interview"
...
tell application "Terminal"
  ...
  delay 0.5
  do script "cd '$WT' && echo '──────── FIX-1 · interview turns · SONNET ────────' && claude --permission-mode bypassPermissions --model sonnet" in front window
end tell
OSA
sleep 6
osascript <<OSA
...
OSA
echo "Staged FIX-1 (Sonnet). Review directive, press Return."
